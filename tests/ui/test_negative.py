...
class TestNegativeUI:
    ...
    def test_leave_review_invalid_text(self, registered_user, page, review_text, create_movie):
        login_page = CinescopeLoginPage(page)
        movie_page = CinescopeMovieDetailPage(page)

        login_page.open()
        login_page.login(registered_user.email, registered_user.password)
        page.goto(MOVIE_URL_TEMPLATE.format(create_movie["id"]))
        page.locator("[data-qa-id='movie_rating_select']").wait_for(state="visible", timeout=15000)
